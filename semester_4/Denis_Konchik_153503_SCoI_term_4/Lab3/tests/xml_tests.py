import unittest
from serializer_Konchik import XmlSerializer
from tests.code import (circle_area, square_area, generator, bts, bts_arr, sum_func,
...
class BaseXmlTestCase(unittest.TestCase):
    def setUp(self) -> None:
        self.serializer = XmlSerializer()
        self.alias = lambda x: self.serializer.loads(self.serializer.dumps(x))


class PrimitivesTestCase(BaseXmlTestCase):
    def test_int(self):
...
class CollectionsTestCase(BaseXmlTestCase):
    def test_list(self):
        self.assertEqual([], self.alias([]))
...
class FunctionsTestCase(BaseXmlTestCase):
...
class IterableTestCase(BaseXmlTestCase):
...
class ClassesTestCase(BaseXmlTestCase):
...
class ObjectTestCase(BaseXmlTestCase):
...
class ScopesTestCase(BaseXmlTestCase):
