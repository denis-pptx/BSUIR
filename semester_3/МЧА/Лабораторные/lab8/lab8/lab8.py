...
def fd3(x): return 3/(8*x**(5/2)*(1+x))+1/(2*x**(3/2)*(1+x)**2)+1/((x**(1/2))*(1+x)**3)
def fd4(x): return -15/(16*x**(7/2)*(1+x))-9/(8*x**(5/2)*(1+x)**2)-3/(2*x**(3/2)*(1+x)**3)-3/((x**(1/2))*(1+x)**4)
M2deLR, M4deLR = 0, 0
...
# def fd4(x): return -15*x**4/(-x**2+1)**(7/2)-18*x**2/(-x**2+1)**(5/2)-3/(-x**2+1)**(3/2)
# M2deLR, M4deLR = 0, 0

# L, R, DerXdot = 0, np.pi, np.pi
# BIG_CONST = np.pi
# def f(x): return BIG_CONST * np.sin(BIG_CONST * x)
# def F(x): return -np.cos(BIG_CONST * x)
# def fd1(x): return BIG_CONST**2 * np.cos(BIG_CONST * x)
# def fd2(x): return -BIG_CONST**3 * np.sin(BIG_CONST * x)
# def fd3(x): return -BIG_CONST**4 * np.cos(BIG_CONST * x)
# def fd4(x): return BIG_CONST**5 * np.sin(BIG_CONST * x)
...
print()
def delta(derappr): return np.ceil(abs(derappr - fd1(DerXdot)) * (1 / (DerEps / 10))) * (DerEps / 10)
print("First Derivative = " + DerFormatString.format( fd1(DerXdot) ))
print("ViaEstimation    = " + DerFormatString.format( DerivativeFirstViaEstimation(f, DerXdot) ), \
      " | delta = " + DerFormatString.format( delta(DerivativeFirstViaEstimation(f, DerXdot)) ))
print("ViaTenInMinus5   = " + DerFormatString.format( DerivativeFirstViaTenInMinus5(f, DerXdot) ), \
      " | delta = " + DerFormatString.format( delta(DerivativeFirstViaTenInMinus5(f, DerXdot)) ))
print()
def delta(derappr): return np.ceil(abs(derappr - fd2(DerXdot)) * (1 / (DerEps / 10))) * (DerEps / 10)
print("Second Derivative = " + DerFormatString.format( fd2(DerXdot) ))
print("ViaEstimation     = " + DerFormatString.format( DerivativeSecondViaEstimation(f, DerXdot) ), \
      " | delta = " + DerFormatString.format( delta(DerivativeSecondViaEstimation(f, DerXdot)) ))
print("ViaTenInMinus4    = " + DerFormatString.format( DerivativeSecondViaTenInMinus4(f, DerXdot) ), \
      " | delta = " + DerFormatString.format( delta(DerivativeSecondViaTenInMinus4(f, DerXdot)) ))
print()
...
print(("ViaMiddleRectangles = " + IntFormatString + " | delta = " + IntFormatString).format( intappr , delta(intappr) ))
intappr = IntegralViaTrapezoidsViaEstimation(f, L, R)
print(("ViaTrapezoids       = " + IntFormatString + " | delta = " + IntFormatString).format( intappr , delta(intappr) ))
intappr = IntegralViaSimpsonViaEstimation(f, L, R)
print(("ViaSimpson          = " + IntFormatString + " | delta = " + IntFormatString).format( intappr , delta(intappr) ))
print()
